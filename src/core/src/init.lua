if not sdl then
    error("SDL2 module needed")
end
if not gl then
    error("OpenGL module needed")
end
...
--- @class core.Context
--- @field audio core.audio.AudioSystem
...
function core.init(settings)
    ...
    local projPath = nil
    if args[2] == 'run' or args[2] == 'build' then
        projPath = args[3]
    elseif args[2] then
        projPath = args[2]
    end
    if not projPath then
        ...
        core.projectFs = Filesystem.create(projPath)
    end
