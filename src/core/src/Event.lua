...
function Event.create()
    ...
    e.callbacks = {
        ...
        [sdl.WINDOWEVENT] = function()
            local ev, wid, d1, d2 = e.handle:window_event()
            e.window.id = wid
            e.window.data[1] = d1
            e.window.data[2] = d2
        ...
        [sdl.MOUSEBUTTONDOWN] = function()
            e.name = 'mouse button'
            local which,btn,x,y = e.handle:mouse_button_event()
            e.mouse.which = which
            e.mouse.x = x
            e.mouse.y = y
        ...
        [sdl.MOUSEMOTION] = function()
            e.name = 'mouse motion'
            local which, x, y, xrel, yrel = e.handle:mouse_motion_event()
            e.mouse.which = which
            e.mouse.x = x
            e.mouse.y = y
        ...
        [sdl.MOUSEWHEEL] = function()
            e.name = 'mouse wheel'
            local which, x, y, flipped = e.handle:mouse_wheel_event()
            e.mouse.which = which
        ...
        [sdl.KEYDOWN] = function()
            e.name = 'key down'
            local scan,pressed,rpt,key,mod = e.handle:keyboard_event()
            e.key.scancode = scan
            e.key.keycode = key
            e.key.mod = mod
        ...
        [sdl.TEXTEDITING] = function()
            e.name = 'text editing'
            local text, start, length = e.handle:text_edit_event()
            e.text.text = text
            e.text.start = start
        ...
        [sdl.TEXTINPUT] = function()
            e.name = 'text input'
            local text = e.handle:text_event()
            e.text.text = text
        end,
        [sdl.AUDIODEVICEADDED] = function()
            e.name = 'audio device'
            local which = e.handle:audio_device_event()
            e.audio.added = true
            e.audio.which = which
        end,
        ...
        [sdl.CONTROLLERDEVICEADDED] = function ()
            ...
            local which = e.handle:gamepad_device_event()
...
function Event:getType()
    return types[self.handle:get_type()]
end

--- @return window_ev_types, integer, integer, integer
function Event:getWindowEvent()
    local wev, wid, d1, d2 = self.handle:window_event()
